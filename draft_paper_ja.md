...
電場増強の計算にはGans近似（E∝gap^(-2.5)）を使用したが、脱分極因子LZ=0.039はGans脱分極公式の解析的検証によりprolate spheroid AR≈6.5に対応することが確認された（e=0.9881, L_a=0.0386≈0.039）。当初cylinder AR=4として記録されていたが不整合であった。また媒質誘電率EPS_M=2.25（SiO₂として設定）の根拠も記録されていない。de Graaf et al.（2018）の実験環境はAl₂O₃基板（ε≈3.1）であり設定値との乖離がある。Gans理論はダイマーナノギャップには不適用であり、正確な電場増強値にはFDTD計算が必要である。
...
（1）脱分極因子LZ=0.039：Gans脱分極公式による解析的検証によりprolate spheroid AR≈6.5に対応することが確認されている（e=0.9881, L_a=0.0386≈0.039）。当初cylinder AR=4として記録されていたが、Gans公式による計算値（Lz≈0.075）と一致せず不整合であった。
